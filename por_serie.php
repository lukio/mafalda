...
if (is_number($ncelda) && !is_text($ncelda)){
	...
	//conecta a Ensayo, debe corregirse cuando este listo el nuevo programa de ensayos
	// Muere el DSN Ensayos. 
	/*
	if (!($id_db_ensayo = odbc_connect($db_ensayo, "", ""))){
		echo 'Fallo conexion base de datos'.$connecstring .'';
		exit();
	}
	 */

	//consulta a dbms  . Selecciona campo NRO_SERIE from tabla ENSAYOS
	//$query = "SELECT e.RANGO_FIN, e.MODELO, i.ImpSG, i.ImpRB, i.Lote, dh.Cero, dh.Pendiente, dh.R2, em.ID_Grupo FROM ENSAYOS e, Impedancias i, DataHorno dh, EMBALADO em WHERE e.NRO_SERIE=$ncelda AND i.Serie=$ncelda AND dh.Serie=$ncelda";
